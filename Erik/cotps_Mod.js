...
const main = async _ => {
    ...
    const makeTransactions = async (browser, { routine = 'makeTransactions', page = 'loadTransactionHall', retry = routineRetries } = {}) => {
        try {
            ...
            // Derk - 25Apr22 - Added outer loop; continue until user stops it
            do {
				...
				// Derk - 25Apr22 - Added delay, log, and end of do-while loop
				console.log('============= Awaiting New Transactions =============', moment());
				await delay({ ms: intervalTime * 60000 })
				console.log('=---=---=---= Refreshing Wallet Balance =---=---=---=', moment());
				//
				//	HELP! Right here, I need to reload the page its on (from line 118)
				//
			} while (true)
        }
        catch (err) {
    ...
    const checker = async (browser, { routine = 'checker', page = 'loadTransactionHall', retry = routineRetries } = {}) => {
        try {
    ...
    try {
		...
		// Keep looping until I force a stop in the console
		while (true) 
		{
			do 
			{
				console.log('=---=---=---= Attempting Transaction =---=---=---=', moment());
				insufficientBalance = checker();
...
(async function invoker() {
    ...
    if (intervalEnabled) {
        ...
        while (true) {
            await main()
            // Derk - 25Apr22 - Added *6 to delay; using intervalTime 
            // now for transaction wait above, which will probably be 
            // set to 5 minutes, so use 6 times for between runs (30 
            // minutes), although it will never get to it because it 
            // runs in a constant 5 minute loop now.
            await delay({ ms: intervalTime * 60000 * 6 })
        }
    }
    else {
        await main()
